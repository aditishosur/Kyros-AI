...
def shell() -> None:
    with st.sidebar:
        st.markdown('<div class="brand"><div class="name">PULSEOPS</div><div class="tag">Predictive API Intelligence</div></div>', unsafe_allow_html=True)
        for page in PAGES[1:]:
            label = page.upper()
...
def api_selector(label: str = "Select API") -> tuple[list[dict], int]:
    ...
    labels = {api["id"]: f'{api["name"]}  {api["method"]} {api["endpoint"]}' for api in apis}
    api_id = st.selectbox(label, ids, format_func=lambda value: labels[value], index=ids.index(st.session_state.api_id))
    st.session_state.api_id = api_id
    return apis, api_id
...
def landing() -> None:
    ...
    targets = ["Overview", "Traffic Forecast", "Incidents & Root Cause", "What-If Simulator"]
    for col, (num, title, body), target in zip(cols, features, targets):
        with col:
            st.markdown(f'<div class="feature-card"><div class="eyebrow">{num}</div><h3>{title}</h3><p class="muted">{body}</p></div>', unsafe_allow_html=True)
...
def overview_page() -> None:
    ...
    st.markdown('<div class="muted">Real-time API health, performance and predictive risk.</div>', unsafe_allow_html=True)
    st.markdown(
    ...
    view = api_rows[["name", "status", "risk_score", "traffic_delta", "latency", "error_rate", "anomaly", "endpoint"]].rename(
        columns={
            "name": "API",
            "status": "Status",
            "risk_score": "Risk",
    ...
    if ts_frames:
        ...
        perf = combined.groupby("timestamp", as_index=False).agg(response_time_ms=("response_time_ms", "mean"), error_rate=("error_rate", "mean"))
        with left:
            st.plotly_chart(line_chart(traffic, "timestamp", [("request_count", "Requests")], "Traffic Overview", 410), use_container_width=True)
        with right:
            st.plotly_chart(line_chart(perf, "timestamp", [("response_time_ms", "Latency"), ("error_rate", "Errors %")], "Performance Overview", 410), use_container_width=True)
...
def intelligence_page() -> None:
    ...
    st.markdown(f'<div class="muted">{api["method"]} {api["endpoint"]} owned by {api["owner"]}</div>', unsafe_allow_html=True)
    c1, c2 = st.columns([.82, 1.18])
    with c1:
        st.plotly_chart(gauge(metrics["risk_score"]), use_container_width=True)
        st.markdown(f'<div class="panel">{status_badge(metrics["status"])}<br><br>{risk_bar(metrics["risk_score"])}</div>', unsafe_allow_html=True)
    with c2:
        st.markdown("### Risk Contributors")
        for name, value in metrics["contributors"].items():
            st.markdown(f'<div class="panel" style="margin-bottom:.45rem;display:flex;justify-content:space-between;"><span>{name}</span><strong>+{value:.1f}</strong></div>', unsafe_allow_html=True)
    cols = st.columns(6)
    for col, args in zip(
        cols,
        [
    ...
    if not df.empty:
        ...
        with left:
            st.plotly_chart(line_chart(df, "timestamp", [("request_count", "Traffic"), ("response_time_ms", "Latency")], "Traffic and Latency"), use_container_width=True)
        with right:
            st.plotly_chart(line_chart(df, "timestamp", [("error_rate", "Error %"), ("db_latency_ms", "DB latency")], "Errors and Database Pressure"), use_container_width=True)
    st.markdown(
...
def forecast_page() -> None:
    ...
    st.markdown('<div class="muted">Predict API demand before it becomes an operational problem.</div>', unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        <div class="panel">
            ...
            <p class="muted">Recommendation: {"Increase service capacity by approximately " + str(round(max(gap, 0) + 10, 1)) + "% before the projected peak." if gap > 0 else "Current capacity appears sufficient for the forecast window."}</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def root_cause_page() -> None:
    ...
    st.markdown('<div class="muted">Understand why an API is degrading, not just that it is degrading.</div>', unsafe_allow_html=True)
    cols = st.columns(3)
    with cols[0]:
    ...
    with right:
        ...
        st.plotly_chart(bar_chart(list(factors.keys()), list(factors.values()), "Contributing Factors"), use_container_width=True)
    st.markdown(f'<div class="panel"><h3>Recommended Action</h3><p>{incident["recommended_action"]}</p></div>', unsafe_allow_html=True)
    c1, c2 = st.columns(2)
...
def simulator_page() -> None:
    ...
    st.markdown('<div class="muted">Test operational decisions before applying them to production. SIMULATED/PREDICTED, not live infrastructure measurements.</div>', unsafe_allow_html=True)
    ...
    if run or "last_sim" not in st.session_state:
        st.session_state.last_sim = post_json(
            "/simulation/run",
            {"api_id": api_id, "traffic_change": traffic, "capacity_change": capacity, "db_latency_change": db_latency},
        )
...
def registry_page() -> None:
    ...
    status_filter = st.multiselect("Status", ["Healthy", "Degraded", "Critical"], default=["Healthy", "Degraded", "Critical"])
    rows = []
    for api in apis:
        ...
        if search.lower() in f'{row["API"]} {row["Endpoint"]} {row["Owner"]}'.lower() and row["Status"] in status_filter:
            rows.append(row)
    df = pd.DataFrame(rows)
    st.dataframe(df.drop(columns=["id"]) if not df.empty else df, use_container_width=True, hide_index=True)
    if not df.empty:
        api_id = st.selectbox("Open API", df["id"].tolist(), format_func=lambda value: df[df["id"] == value]["API"].iloc[0])
        ...
        with c2.expander("Add API"):
            with st.form("add_api"):
                ...
                if st.form_submit_button("Create"):
                    put_payload = {"name": name, "endpoint": endpoint, "method": method, "owner": owner, "description": desc, "status": "Healthy"}
                    post_json("/apis", put_payload)
        ...
        with c3.expander("Edit / Delete"):
            ...
            with st.form("edit_api"):
                ...
                status = st.selectbox("Status", ["Healthy", "Degraded", "Critical"], index=["Healthy", "Degraded", "Critical"].index(current["status"]) if current["status"] in ["Healthy", "Degraded", "Critical"] else 0)
                if st.form_submit_button("Update"):
                    put_json(f"/apis/{api_id}", {"name": name, "owner": owner, "status": status})
